...
def simulate_NN(seed,N,d,n_test,h_dim,EPOCH,lr,act,model_type,epoch_choice=None,lam_choice=None,cond_shred_choice=None):
    np.random.seed(seed)
    ...
    return  cov_mat_ls, U_ls, y_test_ls


# def simulate_NN_real(seed,X,Y,X_test,n_test,h_dim,EPOCH,lr,act,epoch_choice=None,lam_choice=None,cond_shred_choice=None):
#     N = X.shape[0]
#     d = X.shape[1]
#     X_tf = tf.convert_to_tensor(X)
#     Y_tf = tf.convert_to_tensor(Y)

#     X_test_tf = tf.convert_to_tensor(X_test[:n_test,:])   
